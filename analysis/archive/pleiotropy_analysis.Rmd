...
nrow(data %>%
  filter(annotation != 'pLoF|missense|LC' & n_phewas_sig > 1) )
```
...
sum_data <- sum_data %>%
  mutate(
    ...
    # z = (var_n_sig - mean_n_sig) / sqrt((2 * mean_n_sig^2) / cnt)
    ) %>%
  mutate(
...
fit <- data %>%
  group_by(annotation) %>%
  do(tidy(glm(n_phewas_sig ~ 1, family = poisson, data = .)))

# Summary of the fit
summary(fit)

# Extracting the mean (lambda)
lambda <- coef(fit)

cat("Estimated Mean (Lambda):", lambda, "\n")
cat("Sample Variance:", var(data), "\n")

# Using deviance to assess fit
...
# Interpretation
if (p_value > 0.05) {
  cat("Fail to reject the null hypothesis. The model fits the data well, suggesting mean and variance are consistent with a Poisson distribution.\n")
} else {
  cat("Reject the null hypothesis. The model does not fit the data well, suggesting mean and variance are not consistent with a Poisson distribution.\n")
}
```


```{r}
observed <- table(data$n_phewas_sig)
expected <- dpois(as.numeric(names(observed)), lambda = mean(data$n_phewas_sig)) 
chisq.test(observed, expected)

data %>%
  group_by(annotation) %>%
  dplyr::summarize(cnt = as.numeric(names(table(n_phewas_sig))),observed = table(n_phewas_sig), lambda = mean(n_phewas_sig)) %>%
  mutate(expected = dpois(cnt, lambda = lambda) ) %>%
  dplyr::summarize(tidy(chisq.test(observed, expected)))
```



```{r}
pois_data <- data.frame(x = as.numeric(rep(0:30,3)), 
                        y = as.numeric(c(sapply(0:30, dpois, lambda=unlist(sum_data[sum_data$annotation == 'pLoF', 'mean_n_sig']))*unlist(sum_data[sum_data$annotation == 'pLoF', 'cnt']), 
                                          ...
                                          sapply(0:30, dpois, lambda=unlist(sum_data[sum_data$annotation == 'synonymous', 'mean_n_sig']))*unlist(sum_data[sum_data$annotation == 'synonymous', 'cnt']))), 
                        annotation = rep(c('pLoF', 'missense|LC', 'synonymous'), each=31)) %>%
                     mutate(annotation = factor(annotation, levels = annotation_types))

pois_data <- data.frame(x = as.numeric(rep(0:30,3)), 
                        y = as.numeric(c(sapply(0:30, dpois, lambda=unlist(sum_data[sum_data$annotation == 'pLoF', 'mean_n_sig'])), 
                                          sapply(0:30, dpois, lambda=unlist(sum_data[sum_data$annotation == 'missense|LC', 'mean_n_sig'])), 
                                          sapply(0:30, dpois, lambda=unlist(sum_data[sum_data$annotation == 'synonymous', 'mean_n_sig'])))), 
                        annotation = rep(c('pLoF', 'missense|LC', 'synonymous'), each=31)) %>%
                     mutate(annotation = factor(annotation, levels = annotation_types))
```
...
x <- data %>%
  group_by(annotation) %>%
  dplyr::mutate(lambda = mean(n_phewas_sig)) %>%
  mutate(expected = dpois(n_phewas_sig, lambda = lambda) ) %>%
  group_by(annotation) %>%
  dplyr::summarize(tidy(t.test(observed, expected)))
```
...
```{r}
icd_domain <- read_delim(paste0(data_path, 'icd', '_domain_level_gene_sig_burden_', tranche,'.txt.bgz'), delim='\t', col_types = cols(phenocode = col_character())) %>% filter(!is.na(pheno_group)) 
```
...
table(group_cnt_sum[group_cnt_sum$group_cnt>1,'annotation'])
...
disease_domain_sum <- gene_point_check %>%
  ...
  select(Gene = gene_symbol, Annotation = annotation, N_domain = group_cnt, A, C, D, E, `F`, G, H1, I, J, K, L, M, N, Q, R ) %>%
  arrange(., desc(N_domain))
write_csv(disease_domain_sum, paste0(result_path, 'disease_domain_most_pleiotropic_genes.csv'))
```

```{r}
complex_pheno_upset(group_type = 'icd', test_type = 'burden', max_size = 10, save = T, height = 10, width = 20)
```
...
```{r}
save = T
complex_pheno_upset(group_type = 'pheno', test_type = 'burden', max_size = 80, save = save, height = 4, width = 10)
```
...
gene_list_pleiotropy_figure <- function(test, fig_type, save_plot=T){
  ...
  gene_dd <- load_ukb_file('forKonrad_sig31kDNM_consensus_genes_2021_01_12.txt', subfolder = 'analysis/') %>% dplyr::select(gene_symbol = symbol) 
...
```{r}
gene_point_check <- read.csv(paste0(data_path, 'gene_point_check.csv'), sep = ',')
gene_view <- gene_point_check %>% dplyr::select(2:4, 13:15, 20:21)
```

```{r}
table(gene_view[gene_view$gene_symbol == 'ANKRD12' & gene_view$annotation == 'pLoF', 'pheno_group'])
```
...
```{r}
gene_table <- data %>%
  pivot_wider(id_cols = c('gene_symbol', 'gene_id'), values_from = n_phewas_sig, names_from = annotation) %>% 
  filter(pLoF + `missense|LC` > 1)
```


```{r}
n_pleiotropic <- dim(unique((data %>% filter(n_phewas_sig>1) %>% dplyr::select(gene_symbol))))[1]
...
# gene_view <- gene_view %>% filter(new_keep_pheno_unrelated == 'True')
...
```{r}
permute_data <- data.frame(prop_pleiotropy = c(unlist(plof_1), unlist(plof_2), unlist(plof_3), unlist(plof_4), unlist(mis_1), unlist(mis_2), unlist(mis_3), unlist(mis_4), unlist(syn_1), unlist(syn_2), unlist(syn_3), unlist(syn_4), unlist(combined_1), unlist(combined_2), unlist(combined_3), unlist(combined_4)), annotation = rep(c('pLoF', 'missense|LC', 'synonymous', 'pLoF|missense|LC'), each =400), interval =rep(rep(c('(0.0001, 0.001]', '(0.001, 0.01]', '(0.01, 0.1]', '(0.1, 1]'), each=100), times=4))
mean_permute <- permute_data %>%
  group_by(annotation, interval) %>%
  summarize(mean_permute = mean(prop_pleiotropy))%>%
  mutate(annotation = factor(annotation, levels = annotation_types2),
...
## APOB
```{r}
apob_pheno <- read_delim('~/Downloads/apob_gene_sig_phenos.txt.bgz', delim = '\t')
```


## SKAT vs. Burden
```{r}
ratio <- read_delim('~/Downloads/skat_burden_comparison_over_5.txt.bgz', delim = '\t', col_types = cols(log_skat_burden_p_ratio = col_double())) %>%
  select(1:6, 10, 12:16)
```

```{r}
apob_ratio <- read_delim('~/Downloads/skat_burden_comparison_APOB.txt.bgz', delim = '\t', col_types = cols(log_skat_burden_p_ratio = col_double())) %>%
  select(1:6, 10, 12:16)
```