...
######## Simulations
pheno_corr_test <- function(n_ind, n_var, c, r, pi, sigma, mle = TRUE, null=TRUE, mode = 'linear', rho = 0.5, sigma_eps = sigma){
  AC <- get_ac_mat(n_var)
...
pheno_corr_sims <- function(times, n_ind, n_var, c, r, pi, sigma, mle = TRUE,
                            null=TRUE, mode='linear',  rho = 0.5, sigma_eps = sigma,
                            write = TRUE, output=NULL, name=NULL){
  if(null){
  ...
  for(i in 1:nrow(par_set)){
    ...
    temp <- replicate(n = times, pheno_corr_test(n_ind=n_ind, n_var=n_var, c=c, r=r, pi=pi, sigma=sigma,
                                                 mle=mle, null=null, mode = mode, rho = rho, sigma_eps = sigma_eps), simplify = FALSE)
