...
combined_1 <- read_pickle_file(paste0(data_path,"pvalue_permutation/gene_plof_missense_lc_(0.01%, 0.1%]_proportion"))
combined_2 <- read_pickle_file(paste0(data_path,"pvalue_permutation/gene_plof_missense_lc_(0.1%, 1%]_proportion"))
combined_3 <- read_pickle_file(paste0(data_path,"pvalue_permutation/gene_plof_missense_lc_(1%, 10%]_proportion"))
combined_4 <- read_pickle_file(paste0(data_path,"pvalue_permutation/gene_plof_missense_lc_(10%, ∞)_proportion"))

true <-  read.csv(paste0(data_path, 'gene_phewas_burden_sig_count_239.csv'), sep = '\t')
true_sum <- true %>%
  # filter(annotation != 'pLoF|missense|LC') %>%
  dplyr::mutate(interval = get_freq_interval(CAF)) %>%
  dplyr::group_by(annotation, interval) %>%
  ...
  dplyr::mutate(annotation = factor(annotation, levels = annotation_types2),
...
permute_data <- data.frame(prop_pleiotropy = c(unlist(plof_1), unlist(plof_2), unlist(plof_3), unlist(plof_4),
                                               unlist(mis_1), unlist(mis_2), unlist(mis_3), unlist(mis_4),
                                               unlist(syn_1), unlist(syn_2), unlist(syn_3), unlist(syn_4),
                                               unlist(combined_1), unlist(combined_2), unlist(combined_3), unlist(combined_4)
                                               ),
                           annotation = rep(c('pLoF', 'missense|LC', 'synonymous', 'pLoF|missense|LC'), each =400),
                           interval =rep(rep(c('(0.0001, 0.001]', '(0.001, 0.01]', '(0.01, 0.1]', '(0.1, 1]'), each=100), times=4))
mean_permute <- permute_data %>%
  dplyr::group_by(annotation, interval) %>%
  dplyr::summarize(mean_permute = mean(prop_pleiotropy))%>%
...
plofmis <- read_pickle_file(paste0(data_path,"pvalue_permutation/gene_plof_missense_lc_proportion"))
...
permute_data <- data.frame(prop_pleiotropy = c(unlist(plof), unlist(mis), unlist(syn), unlist(plofmis)),
                           annotation = rep(c('pLoF', 'missense|LC', 'synonymous', 'pLoF|missense|LC'), each =100))
mean_permute <- permute_data %>%
