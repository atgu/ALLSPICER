...
p1<- figure_beta_triplets_all_annt(alb_info  %>% filter(AF < 1e-4),
  ...
  geom_point(data = alb_info %>% filter(locus == 'chr4:73412072') %>%
               ...
               mutate(annotation = if_else(annotation %in% c('missense', 'LC'), 'missense|LC', annotation))%>%
               mutate(annotation = factor(annotation, levels=annotation_types)), aes(x = BETA_30600, y=BETA_30680), size=5, pch=1, color='blue') +
  geom_text_repel(data = alb_info %>% filter(locus == 'chr4:73412072') %>%
                    mutate(annotation = if_else(annotation %in% c('missense', 'LC'), 'missense|LC', annotation)) %>%
                    pivot_wider(names_from = phenocode,  values_from = c('BETA', 'Pvalue'), id_cols = c('locus', 'alleles','AF', 'gene', 'annotation')) %>%
                    mutate(annotation = if_else(annotation %in% c('missense', 'LC'), 'missense|LC', annotation))%>%
                    mutate(annotation = factor(annotation, levels=annotation_types)), aes(x = BETA_30600, y=BETA_30680, label = 'chr4:73412072:A:G'), size=3, color='blue', face = 'bold', vjust = 1, hjust = 0.5)
...
png(paste0(figure_path,'figure5.png'), height = 5, width = 7.5, units = 'in', res = 300)
print(figure)
dev.off()
